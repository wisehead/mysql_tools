...
#include <assert.h>
#include <map>
#include <string>//chenhui
#include <iostream>//chenhui
...
bool is_the_filed_varchar(int type)
{
    return (type == musername || type == mrealname || type == mimgurl || type == mpower 
            || type == mcreate_user|| type == mextinfo || type == mextcount || type == midcard 
            || type == mmobile || type == mpassword || type == morg_name || type == mp_card1 
            || type == mp_card2 || type == mp_card3 || type == mp_card4 || type == maccount_id );
}
...
int main(int argc, char** argv)
{
    ...
    uint64_t index_1_page_count = 0;
    uint64_t index_2_page_count = 0;
    uint64_t index_3_page_count = 0;
    uint64_t index_4_page_count = 0;
    uint64_t index_5_page_count = 0;
    uint64_t index_6_page_count = 0;
    uint64_t index_7_page_count = 0;
    uint64_t index_unknow_page_count = 0;
    ...
    while (page_count < max_page_no) {
        ...
        printf("page->page_no is:%lu, page_prev:%lu, page_next:%lu.", page->page_no,page_prev, page_next);
        if (page_index_id == 0x1263) 
        {
            index_1_page_count++;//chenhui3
            printf("index_1_page_count is:%lu, page_type is:%lu.\n", index_1_page_count, page_type);
        }
        else if (page_index_id == 0x1264) 
        {
            index_2_page_count++;//chenhui3
            printf("index_2_page_count is:%lu, page_type:%lu.\n", index_2_page_count, page_type);
        }
        else if (page_index_id == 0x1265) 
        {
            index_3_page_count++;//chenhui3
            printf("index_3_page_count is:%lu, page_type:%lu.\n", index_3_page_count,page_type);
        }
        else if (page_index_id == 0x1266) 
        {
            index_4_page_count++;//chenhui3
            printf("index_4_page_count is:%lu,page_type:%lu.\n", index_4_page_count,page_type);
        }
        else if (page_index_id == 0x1267) 
        {
            index_5_page_count++;//chenhui3
            printf("index_5_page_count is:%lu, page_type:%lu.\n", index_5_page_count,page_type);
        }
        else if (page_index_id == 0x1268) 
        {
            index_6_page_count++;//chenhui3
            printf("index_6_page_count is:%lu, page_type:%lu.\n", index_6_page_count,page_type);
        }
        else if (page_index_id == 0x1269) 
        {
            index_7_page_count++;//chenhui3
            printf("index_7_page_count is:%lu, page_type:%lu.\n", index_7_page_count,page_type);
        }
        else
        {
            index_unknow_page_count++;//chenhui3
            printf("unkno_count is:%lu, page_index_id is:%lu. page->page_no is:%lu. page_n_recs:%lu.page_type:%lu.\n", 
                    index_unknow_page_count, page_index_id, page->page_no, page_n_recs, page_type);
        }
        if (page_index_id == 0x1263 && page_next != 4294967295)
        {
            page->page_no = page_next;
        }
        /*
        else if (page_index_id == 0 && page_next == 0)
        {
            page->page_no = page_count--;
        }
        */
        ...
        if (page->page_no == 0)
        {
            continue;
            uint64_t rec_len = 0;
            rec_len = mach_read_from_2(page->buf + X_1st_REC_OFFSET);//chenhui2
            ...
            int rec_count = 0;
            //while (cur_ptr + rec_len < page->buf + PAGE_SIZE && rec_count < 1)
            ...
            while (rec_count < 2000)
            {
                ...
                // record_header 5
                // null_bits 3
                // we'll read one more byte. but not use the first byte.
                //cur_ptr = cur_ptr - 7 - 6 - 8 -5 - 4;
                cur_ptr = cur_ptr -5 - 4;
                ...
                //skip for now.
                /*
                if (null_bits_count > 1)
                {
                    unsigned char double_byte = mach_read_from_1(last_cur_ptr + rec_len - 7 - 6 - 8 -5 - 3 -6);
                    if (double_byte == 0x80)
                        cur_ptr = last_cur_ptr + rec_len - 7 - 6 - 8 -5 - 3 - 16;   
                    else
                        cur_ptr = last_cur_ptr + rec_len - 7 - 6 - 8 -5 - 3 - 15;   
                    continue;
                }
                */
...
*/
                ...
                //cur_ptr points to length of record.
                //rec_len =  mach_read_from_2(cur_ptr);
                //printf("xxx rec_len is %d\n", rec_len);
                ...
                */
                ...
                /*
                if (old_cur_ptr + rec_len +1 != cur_ptr)
                {
                    printf("record boundary error! exiting!\n");
                    printf("old_cur_ptr:%x\n", old_cur_ptr);
                    printf("rec_len:%lu\n", rec_len);
                    printf("old_cur_ptr + rec_len:%x\n", old_cur_ptr + rec_len);
                    printf("cur_ptr:%x\n", cur_ptr);
                    //exit(1);
                }
                char line_buf[2048] = {0};
                snprintf(line_buf, 2048, "%lu,%s,%lu,%s,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%s\n", 
                        row_id, skey.c_str(), uid, account.c_str(), identity, start_time, end_time, income_credit, adjust_credit,
                        tax_credit, actual_credit, status, all_credit, all_tax, idcard.c_str());
                printf("yyy: %s\n", line_buf);
                */
                /*
    "id", 
